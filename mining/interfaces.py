...
class WorkerManagerInterface(object):
	...
	def get_user_difficulty(self, worker_name):
		# Gets worker's initial difficulty from database
		wd = dbi.get_user(worker_name)

		log.debug("Found worker: %s" % str(worker_name))
		log.debug("Data: %s" % str(wd))

		try:
			if len(wd) > 6:
				log.debug("Found difficulty: %s" % str(wd['difficulty']))
				if wd['difficulty'] != 0:
					worker_settings = dbi.get_user_settings(wd['id'])
					if not worker_settings is None and worker_settings['custom_diff_enable'] == 1:
						return (True, wd['difficulty'])
					else:
						return (False, wd['difficulty'])
		except:
...
class ShareManagerInterface(object):
	...
	def on_submit_block(self, is_accepted, worker_name, block_header, block_hash, timestamp, ip, share_diff):
		...
		if is_accepted:
			notify_email = lib.notify_email.NOTIFY_EMAIL()
			notify_email.notify_found_block(worker_name, block_hash, self.block_height, timestamp)
